...
const Card = ({id, name, price, desc, imageUrl, category}) => {
  ...
  return (
    <section className='shadow-md rounded-sm cursor-pointer hover:scale-110 duration-300 transition-transform relative'>
        ...
        <div className='p-2 py-5 flex flex-col gap-3'>
            <a href={`/product/${id}`} className='text-xs underline capitalize text-primary'>{name?.length < 15 ? name : name?.slice(0, 15) + '...'}</a>
